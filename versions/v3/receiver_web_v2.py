...
GEMINI_MODEL   = "gemini-1.5-flash"           # fast + cheap; change to gemini-1.5-pro if needed
...
def _call_gemini(words: list, context: str) -> str:
    ...
    if context.strip():
        system_prompt = GEMINI_PROMPT_WITH_CONTEXT
        user_message  = f"TOPIC / CONTEXT: {context.strip()}\n\nWORDS: {words_str}"
    else:
        system_prompt = GEMINI_PROMPT_NO_CONTEXT
        user_message  = f"WORDS: {words_str}"
